import { loadOpenings, lessonsByTopicId, allLessons } from "./opening-data.js";
import { progress } from "./progress.js";

const COLLECTION = "v1";
const COLLECTION_ID = "watson_v1";
...
function renderTopicLink(lesson) {
  ...
  const href =
    lesson.status === "coming_soon"
      ? "#"
      : `opening-study.html?collection=v1&lesson=${lesson.id}`;
  const cls =
    lesson.status === "coming_soon"
      ? "toc-topic soon"
...
function renderChapter(chapter, topicMap) {
  const topics = chapter.topics
    .map((t) => renderTopicLink(topicMap.get(t.id)))
    .join("");
  const chLabel = chapter.number ? `Ch. ${chapter.number}` : "";
  return `
    <section class="toc-chapter" id="${chapter.id}">
...
function renderPart(part, topicMap) {
  const chapters = part.chapters.map((ch) => renderChapter(ch, topicMap)).join("");
...
async function main() {
  ...
  const toc = { parts: data.parts || [] };
  const topicMap = lessonsByTopicId(data);
  const lessons = allLessons(data);

  document.getElementById("toc-tree").innerHTML = toc.parts
    .map((p) => renderPart(p, topicMap))
    .join("");

  // Scroll to chapter if linked from old watson-chapter.html URLs
  const hash = location.hash.slice(1);
  if (hash) {
    document.getElementById(hash)?.scrollIntoView({ behavior: "smooth", block: "start" });
  }

  document.querySelectorAll(".toc-topic.soon a").forEach((a) => {
    a.addEventListener("click", (e) => e.preventDefault());
  });

  const live = lessons.filter((l) => l.status !== "coming_soon").length;
  ...
  const studied = progress.countSolved(
    COLLECTION_ID,
    lessons.filter((l) => l.status !== "coming_soon").map((l) => l.id)
  );
  const attempted = lessons.filter(
    (l) => l.status !== "coming_soon" && progress.isAttempted(COLLECTION_ID, l.id) && !progress.isSolved(COLLECTION_ID, l.id)
  ).length;
  ...
  document.getElementById("reset-btn")?.addEventListener("click", () => {
    if (confirm("Reset all Watson Vol. 1 progress?")) {
...
main().catch((err) => {
  ...
  document.getElementById("toc-tree").innerHTML = `
    <p style="color:var(--error)">Could not load Watson Vol. 1 course.</p>
  `;
});
