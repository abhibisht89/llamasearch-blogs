/**
 * Endgame section grid — no chess.js dependency (list pages have no importmap).
 */
import { progress, migrateProgressToSequential } from "./progress.js";

export function createEndgameSectionGrid(config) {
  const { sectionId, dataUrl, solvePage, pageSize = 100 } = config;
  ...
  function puzzleVisible(id) {
    ...
    if (currentFilter === "solved") return progress.isSolved(sectionId, id);
    if (currentFilter === "unsolved") return !progress.isSolved(sectionId, id);
    return true;
  }
  ...
  function renderHeader() {
    const ids = puzzleIds;
    const solved = progress.countSolved(sectionId, ids);
    ...
    const nextId = firstUnsolvedId(section);
    const continueBtn = document.getElementById("continue-btn");
    if (nextId) {
      continueBtn.href = `${solvePage}?id=${nextId}`;
  ...
  function renderGrid() {
    ...
    const pageIds = filtered.slice(start, start + pageSize);

    grid.innerHTML = pageIds
      .map((id) => {
        const solved = progress.isSolved(sectionId, id);
        const attempted = progress.isAttempted(sectionId, id);
        const cls = ["puzzle-card", solved ? "solved" : "", attempted ? "attempted" : ""]
          .filter(Boolean)
          .join(" ");
        return `
          <a class="${cls}" href="${solvePage}?id=${id}" title="Puzzle #${id}">
  ...
  document.getElementById("reset-btn")?.addEventListener("click", () => {
    if (confirm(`Reset all progress for ${section.title}?`)) {
      progress.resetSection(sectionId);
      renderHeader();
  ...
  main().catch((err) => {
    ...
    const grid = document.getElementById("puzzle-grid");
    if (grid) {
      grid.innerHTML = `<p style="color:var(--error)">Could not load puzzles. Run a local server from this folder.</p>`;
    }
  });
}
