/**
 * Miniature f3/f6 grid — loads data/sections/miniature_f3_f6.json (not puzzles.json).
 * 100 puzzles fit on one page; pagination hidden when count <= 100.
 */
...
function getFilteredIds() {
  return sortedPuzzleIds(section).filter(puzzleVisible);
}
...
function renderHeader() {
  ...
  if (nextId) {
    continueBtn.href = `solve-miniature-f3-f6.html?id=${nextId}`;
  ...
  grid.innerHTML = pageIds
    .map((id) => {
      ...
      const cls = ["puzzle-card", solved ? "solved" : "", attempted ? "attempted" : ""]
        ...
        .join(" ");
      return `
        <a class="${cls}" href="solve-miniature-f3-f6.html?id=${id}" title="Puzzle #${id}">
...
function renderPagination() {
  ...
  // Single page for 100 puzzles — hide pagination controls.
  if (total <= PAGE_SIZE) {
    nav.hidden = true;
    nav.innerHTML = "";
    return;
...
document.getElementById("reset-btn")?.addEventListener("click", () => {
  if (confirm("Reset all progress for Miniature f3/f6?")) {
