import { loadOpenings, loadTocV3, lessonsByTopicId } from "./opening-data.js";
import { progress } from "./progress.js";

const COLLECTION = "v3";
const COLLECTION_ID = "watson_v3";
...
function renderTopicLink(lesson) {
  ...
  const href =
    lesson.status === "coming_soon"
      ? "#"
      : `opening-study.html?collection=v3&lesson=${lesson.id}`;
  const cls =
    lesson.status === "coming_soon"
      ? "toc-topic soon"
...
async function main() {
  const [data, toc] = await Promise.all([loadOpenings(COLLECTION), loadTocV3()]);
  ...
  document.getElementById("reset-btn")?.addEventListener("click", () => {
    if (confirm("Reset all Watson Vol. 3 progress?")) {
...
main().catch((err) => {
  ...
  document.getElementById("toc-tree").innerHTML = `
    <p style="color:var(--error)">Could not load Watson Vol. 3 course.</p>
  `;
});
