import { loadOpenings, loadTocV2, lessonsByTopicId } from "./opening-data.js";
import { progress } from "./progress.js";

const COLLECTION = "v2";
const COLLECTION_ID = "watson_v2";
...
function renderTopicLink(lesson) {
  ...
  const href =
    lesson.status === "coming_soon"
      ? "#"
      : `opening-study.html?collection=v2&lesson=${lesson.id}`;
  const cls =
    lesson.status === "coming_soon"
      ? "toc-topic soon"
...
function renderPart(part, topicMap) {
  ...
  const title = part.title
    ? `<h2 class="toc-part-title">${part.title}</h2>`
    : "";
  return `
    <div class="toc-part">
      ${title}
...
async function main() {
  const [data, toc] = await Promise.all([loadOpenings(COLLECTION), loadTocV2()]);
  ...
  const attempted = lessons.filter(
    (l) => l.status === "live" && progress.isAttempted(COLLECTION_ID, l.id) && !progress.isSolved(COLLECTION_ID, l.id)
  ).length;
  ...
  document.getElementById("reset-btn")?.addEventListener("click", () => {
    if (confirm("Reset all Watson Vol. 2 progress?")) {
...
main().catch((err) => {
  ...
  document.getElementById("toc-tree").innerHTML = `
    <p style="color:var(--error)">Could not load Watson Vol. 2 course.</p>
  `;
});
