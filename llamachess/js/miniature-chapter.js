...
const PAGE_SIZE = 100;
const chapterSlug = new URLSearchParams(window.location.search).get("chapter") || "f3_f6";
...
function renderHeader() {
  ...
  document.title = `${section.title} — LlamaChess`;
  document.getElementById("breadcrumb-chapter").textContent = chapterMeta.title;
  ...
  if (nextId) {
    continueBtn.href = solveHref(nextId);
  ...
  grid.innerHTML = pageIds
    .map((id) => {
      ...
      const cls = ["puzzle-card", solved ? "solved" : "", attempted ? "attempted" : ""]
        ...
        .join(" ");
      return `
        <a class="${cls}" href="${solveHref(id)}" title="Puzzle #${id}">
