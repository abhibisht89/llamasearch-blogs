...
const COLLECTION = "london";
const COLLECTION_ID = "london";
...
function renderLineCard(lesson) {
  const href = `opening-study.html?collection=london&lesson=${lesson.id}`;
...
async function main() {
  ...
  document.getElementById("reset-btn")?.addEventListener("click", () => {
    if (confirm("Reset all London System progress?")) {
...
main().catch((err) => {
  ...
  document.getElementById("line-grid").innerHTML = `
    <p style="color:var(--error)">Could not load London course.</p>
  `;
});
