import { getSection, sortedPuzzleIds, firstUnsolvedId } from "./data.js";
...
function renderHeader() {
  ...
  if (nextId) {
    continueBtn.href = `solve.html?section=${SECTION_ID}&id=${nextId}`;
  ...
  grid.innerHTML = pageIds
    .map((id) => {
      ...
      const cls = ["puzzle-card", solved ? "solved" : "", attempted ? "attempted" : ""]
        ...
        .join(" ");
      return `
        <a class="${cls}" href="solve.html?section=${SECTION_ID}&id=${id}" title="Puzzle #${id}">
...
document.getElementById("reset-btn")?.addEventListener("click", () => {
  if (confirm("Reset all progress for Mate in 1?")) {
