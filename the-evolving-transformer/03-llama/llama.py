...
# ============================================================
# Config — LLaMA 3 8B from Touvron et al., 2023 / Meta, 2024
# d_model=4096, N=32, H=32, n_kv_heads=8, vocab=128256, context=8192, d_ff=14336
# ============================================================
@dataclass
class LlamaConfig:
    dmodel: int = 4096           # LLaMA 3 8B
    N: int = 32                  # transformer blocks
    H: int = 32                  # query heads
    n_kv_heads: int = 8          # GQA — 4:1 ratio (32 Q / 8 KV)
    vocabsize: int = 128256      # BPE vocabulary size (LLaMA 3)
    max_seq_len: int = 8192      # context length
    dff: int = 14336             # SwiGLU inner dim (not 4 × dmodel)
...
config = LlamaConfig()  # LLaMA 3 8B — shared by every module below
...
class SwiGLUFeedForward(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
        self.w1 = nn.Linear(config.dmodel, config.dff, bias=False)  # (B,S,dmodel) → (B,S,dff) gate
        self.w2 = nn.Linear(config.dmodel, config.dff, bias=False)  # (B,S,dmodel) → (B,S,dff) up
        self.w3 = nn.Linear(config.dff, config.dmodel, bias=False)  # (B,S,dff) → (B,S,dmodel) down
...
class GQABlock(nn.Module):
    def __init__(self, config) -> None:
        ...
        self.wq = nn.Linear(config.dmodel, config.H * self.dk, bias=False)  # dmodel → H*dk
        ...
        self.wo = nn.Linear(config.dmodel, config.dmodel, bias=False)  # dmodel → dmodel
    ...
    def forward(self, x, rope, mask):
        ...
        key = GQABlock.repeat_kv(key, self.n_rep)  # (B, H, S, dk)
        value = GQABlock.repeat_kv(value, self.n_rep)  # (B, H, S, dk)
...
class LLAMA(nn.Module):
    ...
    def forward(self, x):
        _, S = x.shape  # x: (B, S)
        x = self.wte(x)  # (B, S, dmodel) — no wpe; RoPE handles position in attention
...
if __name__ == "__main__":
    demo_S = 32  # short seq for smoke test — full 8192 context is heavy at 8B scale
    ...
    print(
        "Weights tied:",
        llama.lm_head.weight.data_ptr() == llama.wte.weight.data_ptr(),
    )
